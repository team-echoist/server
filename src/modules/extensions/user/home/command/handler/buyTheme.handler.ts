...
export class BuyThemeHandler extends BaseCommandHandler<BuyThemeCommand> {
  async execute(command: BuyThemeCommand): Promise<void> {
    ...
    const lock = await this.homeService.acquireLock(lockKey, ttl);

    // useScopeExit으로 lock 해제를 예약
    const releaseLock = this.toolService.useScopeExit(() => this.homeService.releaseLock(lock));
